...
public class OsipitiEntity extends TamableAnimal implements IAnimatable, PlayerRideableJumping {
    private AnimationFactory factory = new AnimationFactory(this);
    public OsipitiEntity(EntityType<? extends TamableAnimal> p_27557_, Level p_27558_) {
    ...
    public static AttributeSupplier setAttributes() {
        return TamableAnimal.createMobAttributes()
                ...
                .add(Attributes.MOVEMENT_SPEED, 0.435f)
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return ModEntityTypes.OSIPITI.get().create(serverLevel);
    }

    @Override
    public boolean isFood(ItemStack pStack) {
        return pStack.getItem().equals(Items.GOLDEN_CARROT);
    }
    ...
    public InteractionResult mobInteract(Player p_30412_, InteractionHand p_30413_) {
        ...
        if (this.level.isClientSide) {
            ...
            } else if (itemstack.is(ModItems.DHIRTA.get()) || itemstack.is(ModItems.ILGROSU.get())) {
                if (!p_30412_.getAbilities().instabuild) {
                    itemstack.shrink(1);
                }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.osipiti.walk", true));
            return PlayState.CONTINUE;
        }
        else if(this.isInSittingPose() ){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.osipiti.sit", true));
            return PlayState.CONTINUE;
        }
        else if(!this.isInSittingPose()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.osipiti.idle", true));
            return PlayState.CONTINUE;
        }
    ...
    private PlayState attackPredicate(AnimationEvent event) {
        if(this.swinging && event.getController().getAnimationState().equals(AnimationState.Stopped)) {
            ...
            event.getController().setAnimation(new AnimationBuilder().
                    addAnimation("animation.osipiti.attack", false));
            this.swinging = false;
            playSound(ModSounds.OSIPITI_ATTACK.get());
        }
        return PlayState.CONTINUE;
    }
    ...
    private void positionRider(Entity p_19957_, Entity.MoveFunction p_19958_) {
        ...
        p_19957_.setPos(this.getX() - (double)(1.2F * f),
                this.getY(0.5D) + p_19957_.getMyRidingOffset() + 1D,
                this.getZ() + (double)(1.2F * f1));
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.OSIPITI_IDLE.get();
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return ModSounds.OSIPITI_HIT.get();
    }
