...
public class SwerdarmEntity extends TamableAnimal implements IAnimatable, PlayerRideableJumping {
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return ModEntityTypes.SWERDARM.get().create(serverLevel);
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.swerdarm.walk", true));
            return PlayState.CONTINUE;
        }
        else if(this.isInSittingPose() ){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.swerdarm.sit", true));
            return PlayState.CONTINUE;
        }
        else if(!this.isInSittingPose()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.swerdarm.idle", true));
            return PlayState.CONTINUE;
        }
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.SWERDARM_IDLE.get();
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return ModSounds.SWERDARM_HIT.get();
    }
