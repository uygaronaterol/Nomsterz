...
public class BekorEntity extends TamableAnimal implements IAnimatable, PlayerRideableJumping {
    private AnimationFactory factory = new AnimationFactory(this);
    public BekorEntity(EntityType<? extends TamableAnimal> p_27557_, Level p_27558_) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return ModEntityTypes.BEKOR.get().create(serverLevel);
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bekor.walk", true));
            return PlayState.CONTINUE;
        }
        else if(this.isInSittingPose() ){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bekor.sit", true));
            return PlayState.CONTINUE;
        }
        else if(!this.isInSittingPose()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bekor.idle", true));
            return PlayState.CONTINUE;
        }
    ...
    public void attackEntityWithRangedAttack(LivingEntity target) {
        ...
        if(this.getTarget() instanceof  LivingEntity)  {
            ...
            BekorProjectile arrow = new BekorProjectile(ModEntityTypes.BEKOR_PROJECTILE.get(), this, this.level);
            if(d3 > 10){
