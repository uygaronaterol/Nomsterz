...
public class RoharkEntity extends TamableAnimal implements IAnimatable {
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return ModEntityTypes.ROHARK.get().create(serverLevel);
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.rohark.walk", true));
            return PlayState.CONTINUE;
        }
        else if(this.isInSittingPose() ){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.rohark.sit", true));
            return PlayState.CONTINUE;
        }
        else if(!this.isInSittingPose()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.rohark.idle", true));
            return PlayState.CONTINUE;
        }
    ...
    private PlayState attackPredicate(AnimationEvent event) {
        if(this.swinging && event.getController().getAnimationState().equals(AnimationState.Stopped)) {
            ...
            event.getController().setAnimation(new AnimationBuilder().
                    addAnimation("animation.rohark.attack", false));
            this.swinging = false;
            playSound(ModSounds.ROHARK_ATTACK.get());
        }
        return PlayState.CONTINUE;
    }
    ...
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return ModSounds.ROHARK_HIT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.ROHARK_DEATH.get();
    }
    ...
    public void attackEntityWithRangedAttack(LivingEntity target) {
        ...
        if(this.getTarget() instanceof  LivingEntity)  {
            ...
            RoharkQuillProjectile arrow = new RoharkQuillProjectile(ModEntityTypes.ROHARK_QUILL_PROJECTILE.get(), this, this.level);
            if(d3 > 10){
