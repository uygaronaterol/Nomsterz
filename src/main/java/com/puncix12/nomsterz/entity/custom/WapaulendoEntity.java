...
public class WapaulendoEntity extends TamableAnimal implements IAnimatable {
    private AnimationFactory factory = new AnimationFactory(this);
    public WapaulendoEntity(EntityType<? extends TamableAnimal> p_27557_, Level p_27558_) {
    ...
    public float getSpeed() {
        if(this.getBlockStateOn().is(Blocks.SAND))
            return 0.7f;
        else
            return 0.5f;
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return ModEntityTypes.WAPAULENDO.get().create(serverLevel);
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.wapaulendo.walk", true));
            return PlayState.CONTINUE;
        }
        else if(this.isInSittingPose() ){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.wapaulendo.sit", true));
            return PlayState.CONTINUE;
        }
        else if(!this.isInSittingPose()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.wapaulendo.idle", true));
            return PlayState.CONTINUE;
        }
