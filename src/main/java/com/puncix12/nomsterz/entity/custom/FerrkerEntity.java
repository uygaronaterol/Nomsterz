...
public class FerrkerEntity extends TamableAnimal implements IAnimatable, PlayerRideableJumping {
    private AnimationFactory factory = new AnimationFactory(this);
    public FerrkerEntity(EntityType<? extends TamableAnimal> p_27557_, Level p_27558_) {
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        ...
        if(entity instanceof LivingEntity)
            entity.causeFallDamage(3f,1f,pSource);
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return ModEntityTypes.FERRKER.get().create(serverLevel);
    }

    @Override
    public boolean isFood(ItemStack pStack) {
        return pStack.getItem().equals(Items.CACTUS);
    }
    ...
    public InteractionResult mobInteract(Player p_30412_, InteractionHand p_30413_) {
        ...
        if (this.level.isClientSide) {
            ...
            } else if ( itemstack.is(ModItems.ILGROSU.get()) ) {
                if (!p_30412_.getAbilities().instabuild) {
                    itemstack.shrink(1);
                }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ferrker.walk", true));
            return PlayState.CONTINUE;
        }
        else if(this.isInSittingPose() ){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ferrker.sit", true));
            return PlayState.CONTINUE;
        }
        else if(!this.isInSittingPose()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ferrker.idle", true));
            return PlayState.CONTINUE;
        }
    ...
    private PlayState attackPredicate(AnimationEvent event) {
        if(this.swinging && event.getController().getAnimationState().equals(AnimationState.Stopped)) {
            ...
            event.getController().setAnimation(new AnimationBuilder().
                    addAnimation("animation.ferrker.attack", false));
            this.swinging = false;
            playSound(SoundEvents.TURTLE_SHAMBLE);
        }
        return PlayState.CONTINUE;
    }
