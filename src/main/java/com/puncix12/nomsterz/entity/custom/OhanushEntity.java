...
public class OhanushEntity extends TamableAnimal implements IAnimatable, PlayerRideableJumping {
    private AnimationFactory factory = new AnimationFactory(this);
    public OhanushEntity(EntityType<? extends TamableAnimal> p_27557_, Level p_27558_) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return ModEntityTypes.OHANUSH.get().create(serverLevel);
    }

    @Override
    public boolean isFood(ItemStack pStack) {
        return pStack.getItem().equals(ModItems.RAW_ROHARK_MEAT.get()) || pStack.getItem().equals(ModItems.COOKED_ROHARK_MEAT.get());
    }
    ...
    public InteractionResult mobInteract(Player p_30412_, InteractionHand p_30413_) {
        ...
        if (this.level.isClientSide) {
            ...
            } else if ((itemstack.is(ModItems.COOKED_KASHISOS_MEAT.get()) || itemstack.is(ModItems.RAW_KASHISOS_MEAT.get())) && this.onGround) {
                if (!p_30412_.getAbilities().instabuild) {
                    itemstack.shrink(1);
                }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ohanush.walk", true));
            return PlayState.CONTINUE;
        }
        else if(this.isInSittingPose() ){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ohanush.sit", true));
            return PlayState.CONTINUE;
        }
        else if(!this.isInSittingPose()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ohanush.idle", true));
            return PlayState.CONTINUE;
        }
    ...
    private PlayState attackPredicate(AnimationEvent event) {
        if(this.swinging && event.getController().getAnimationState().equals(AnimationState.Stopped)) {
            ...
            event.getController().setAnimation(new AnimationBuilder().
                    addAnimation("animation.ohanush.attack", false));
            playSound(ModSounds.OHANUSH_ATTACK.get());
            this.swinging = false;
        }
        return PlayState.CONTINUE;
    }
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.OHANUSH_IDLE.get();
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return ModSounds.OHANUSH_HIT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.OHANUSH_DEATH.get();
    }
    ...
    private void positionRider(Entity p_19957_, Entity.MoveFunction p_19958_) {
        ...
        p_19957_.setPos(this.getX() - (double)(f* 0.1f),
                this.getY(0.5D) + p_19957_.getMyRidingOffset() + 3.3D,
