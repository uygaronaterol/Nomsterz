...
public class YonsiEntity extends TamableAnimal implements IAnimatable, PlayerRideableJumping {
    private AnimationFactory factory = new AnimationFactory(this);
    public YonsiEntity(EntityType<? extends TamableAnimal> p_27557_, Level p_27558_) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.yonsi.walk", true));
            return PlayState.CONTINUE;
        }
        else if(this.isInSittingPose() ){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.yonsi.sit", true));
            return PlayState.CONTINUE;
        }
        else if(!this.isInSittingPose()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.yonsi.idle", true));
            return PlayState.CONTINUE;
        }
    ...
    private void positionRider(Entity p_19957_, Entity.MoveFunction p_19958_) {
        ...
        p_19957_.setPos(this.getX() - (double)(f),
                this.getY(0.5D) + p_19957_.getMyRidingOffset() + 1.7D,
                this.getZ() + (double)(f1));
